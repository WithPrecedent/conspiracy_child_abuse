...
def visualize_panel_effects(
    ...
    data = conspiracy_child_abuse.import_data(path = import_path)
    ...
    axes = data.plot.bar(
        ...
        legend = False,
        ylim = (0, 15))
    conspiracy_child_abuse.add_stick_figure(
        axes,
        x = .3,
        y = .21,
        quote = 'No big deal',
    ...
    conspiracy_child_abuse.export_visual(path = export_path)
    plt.show()
    return
